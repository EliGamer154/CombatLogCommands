...
- `/gamerule combatlogcommands:dragoneggpowers true` — while on, the dragon egg **can't be stored in any container at all** (ender chests, chests, shulker boxes, barrels, bundles — nothing; it can only sit in a player's own inventory or be placed as a block), and any player holding a dragon egg in their inventory gets **+5 hearts** and **Strength II**. Both effects go away when they no longer have the egg (or the rule is turned off).
