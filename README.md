...
- **Auto-accept** with `/tpauto`:
  - `/tpauto` — toggle auto-accepting **all** incoming teleport requests to you. While on, anyone who sends you a `/tpa` or `/tpahere` is accepted automatically (the teleport still runs the normal 3-2-1 countdown).
  - `/tpauto <player>` — toggle auto-accept for **just that one player**, so only their requests skip the menu.
  - Auto-accept preferences are per-player and reset on server restart.
...
- `/gamerule combatlogcommands:dragoneggpowers true` — while on, the dragon egg **can't be stored in any container at all** — ender chests, chests, shulker boxes (opened), barrels, **bundles**, and it can't be moved into any container by **hoppers** either. It can only sit in a player's own inventory or be placed as a block. Any player holding a dragon egg in their inventory gets **+5 hearts** and **Strength II**, which go away when they no longer have the egg (or the rule is off).
...
- `/gamerule combatlogcommands:donethupgradesarmor true` — for servers where netherite is disabled. While on, doing the **normal netherite smithing upgrade** (netherite upgrade template + netherite ingot) on a **diamond** armor piece or tool/weapon produces the same diamond item **maxed out with enchantments** instead of netherite gear. Only diamond items are affected. When you take the result, a **"Choose Enchants" menu opens** where you can toggle individual enchants off (e.g. Knockback) before confirming — click the emerald to take your item (or just close the menu; you still get it). The full enchant sets:
